...
class LLMNeedleHaystackTester:
    ...
    async def evaluate_and_log(self, context_length, depth_percent):
        ...
        if self.save_contexts:
            ...
            if not os.path.exists(f'contexts/{model_name}'):
                os.makedirs(f'contexts/{model_name}')

            with open(f'contexts/{model_name}/{context_file_location}_context.txt', 'w') as f:
                f.write(context)
    ...
    def read_context_files(self):
        ...
        # Get all txt files and sort them alphabetically to ensure consistency with previous experiments when supplementing additional corpus
        txt_files = sorted(glob.glob(os.path.join(base_dir, self.haystack_dir, "*.txt")))
