...
class OpenAI(ModelProvider):
    ...
    def __init__(self,
        ...
        Args:
            ...
            model_kwargs (dict): Model configuration. Defaults to {max_tokens: 2048, temperature: 0}.
    ...
    def decode_tokens(self, tokens: list[int], context_length: Optional[int] = None,
                      preserve_sentence: bool = False) -> str:
        """
        Decodes a sequence of tokens back into a text string using the model's tokenizer.
        ...
        # If sentence ending is found, truncate at that position
        if last_end != -1:
            return truncated_text[:last_end + 1].rstrip()

        # If no sentence ending is found, return the original truncated text
        return truncated_text.rstrip()
